    (function () {
      ...
      function keyFromTitle(modal) {
       ...
       function keyFromFilename(img) {
        // Usamos inferKey (validation.js) para que, aunque el nombre del
        // archivo incumpla la regla, podamos detectar el formato deseado
        // y aplicar el overlay correspondiente.
        if (typeof inferKey !== "function") return null;
...
function fitOverlayRect(ov, img, placement, txtRuleKey) {
  ...
  ov.style.display   = "";
      ...
      // Busca el primer item cargado cuyo formato inferido coincida con el pedido
      function findLoadedByKey(targetKey) {
        const items = window.LOADED_ITEMS || [];
        for (const it of items) {
          try {
            if (typeof inferKey === "function" && inferKey(it.name) === targetKey)
              return it;
          } catch (e) { /* ignore */ }
        }
        return null;
